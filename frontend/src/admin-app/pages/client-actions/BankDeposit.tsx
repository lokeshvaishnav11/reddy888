        ...
        amount:
          client?.role === 'admin'
            ...
            : Yup.number()
                ...
                .min(1, 'Amount is required')
                .max(
                  client?.parentBalance?.balance || 0,
                  `Max ${(client?.parentBalance?.balance || 0).toFixed(2)} limit`,
                ),
      }),
    [client],
  )
    ...
    <div className='client-action-page'>
      <div className='client-action-card'>
        ...
        <div className='client-action-heading'>
          <h2>Bank Deposit</h2>
          <p>Transfer chips from {client.parent?.username || 'parent'} to {client.username}</p>
        </div>
