...
const WithdrawStatement = () => {
  ...
  const [pageCount, setPageCount] = React.useState<any>(1)
  ...
  const getAccountStmt = (page: number) => {
    depositWithdrawService
      .getDepositWithdrawLists({ type: 'withdraw', ...filterData, page })
      .then((res) => {
        const data = res?.data?.data
        if (data?.docs) {
          // Paginated response
          setWithdrawStatement(data.docs)
          setPageCount(data.totalPages || 1)
        } else if (Array.isArray(data)) {
          // Array response without pagination
          setWithdrawStatement(data)
          setPageCount(Math.ceil(data.length / 10) || 1)
        } else {
      ...
      .catch((err) => {
        console.error('Error fetching withdraw statement:', err)
        setWithdrawStatement([])
        setPageCount(1)
      })
  }
            ...
            <div className='card-body'>
              <div className='table-responsive'>
                <table className='table table-bordered' id='customers1'>
                  <thead>
                    <tr>
                      <th className='bg2 text-white' style={{ width: '6%', textAlign: 'center' }}>
                        No
                      </th>
                      <th
                      ...
                      <th className='bg2 text-white' style={{ width: '20%', textAlign: 'center' }}>
                        Note
                      </th>
                      <th className='bg2 text-white' style={{ width: '8%', textAlign: 'center' }}>
                        Details
                      </th>
                      <th className='bg2 text-white' style={{ width: '16%', textAlign: 'center' }}>
                        Action
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {withdrawStatement && withdrawStatement.length > 0 ? (
                      withdrawStatement.map((item: any, index: number) => (
                        <tr key={item._id}>
                          ...
                          <td style={{ textAlign: 'center' }}>{item.accountType || item.type}</td>
                          ...
                          <td style={{ textAlign: 'center' }}>{item.remark || item.note || ''}</td>
                          <td style={{ textAlign: 'center' }}>
                            <button
                              data-toggle='modal'
